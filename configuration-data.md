...
* `use_global_custom_ringtone`: this option determines whether the phone uses a global custom ringtone (from an audio file) for incoming calls.  The per-account custom ringtone (see `use_custom_ringtone`) takes precedence when enabled.
  * UI: **Use global custom ringtone** (located in the **Play sounds** section).
  * Type: ***boolean***.
  * Default value: `false`.

* `ring_tone_file`: this option defines the path to the audio file used as the global custom ringtone.
  * This option only makes sense when `use_global_custom_ringtone` is enabled.
  * UI: **Select global custom ringtone file** (located in the **Play sounds** section).
...
<options>
  ...
  <audio>
    ...
    <use_global_custom_ringtone>false</use_global_custom_ringtone>
    <ring_tone_file/>
